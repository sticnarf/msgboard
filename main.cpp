...
int main() {
    ...
    route->addRule(std::regex("/login"),
                   HttpMethod::METHOD_GET, std::make_shared<ForceLoggedOut>(std::make_shared<NewSession>()));
    route->addRule(std::regex("/login"),
                   HttpMethod::METHOD_POST, std::make_shared<ForceLoggedOut>(std::make_shared<CreateSession>()));
    route->addRule(std::regex("/logout"),
                   HttpMethod::METHOD_GET, std::make_shared<ForceLoggedIn>(std::make_shared<DeleteSession>()));

    route->addRule(std::regex("/users/new"),
                   HttpMethod::METHOD_GET, std::make_shared<ForceLoggedOut>(std::make_shared<NewUser>()));
    route->addRule(std::regex("/users/{0,1}"),
                   HttpMethod::METHOD_POST, std::make_shared<ForceLoggedOut>(std::make_shared<CreateUser>()));
